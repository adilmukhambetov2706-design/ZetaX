...
class Upper_frame(CTkFrame):
    def __init__(self, master):
        super().__init__(master, width=1293, height=40, fg_color='transparent')
        color = "#D9D9D9"

        CTkLabel(self, text='Zeta-X', font=('Montserrat SemiBold', 32)).grid(column=0, row=0, padx=52)
        ...
        burger_button = CTkButton(self, image=burger_image, height=42, width=58, fg_color='transparent',
                                  text="")
...
class Middle_frame(CTkFrame):
    def __init__(self, master):
        super().__init__(master, width=1344, height=619, fg_color="#161358", corner_radius=15)

        kroshki_frame = CTkFrame(self, fg_color="transparent")
        kroshki_frame.pack(anchor='nw', pady=(23, 0), padx=25)
        CTkLabel(kroshki_frame, text="Home", font=('Monteserrat SemiBold', 14)).grid(column=0, row=0)
        ...
        CTkLabel(kroshki_frame, text="Bishkek", font=('Monteserrat SemiBold', 14)).grid(column=2, row=0)

        CTkLabel(self, text='Bishkek: Detailed 14-Day & Hourly Forecast',
                 font=('Monteserrat', 48)).pack(anchor='n', pady=(33, 0))

        detailed_24h_frame = CTkFrame(self, fg_color="#130F60", width=1292, corner_radius=15)
        detailed_24h_frame.pack(anchor='n', pady=(42, 0))

        CTkLabel(detailed_24h_frame, text='Bishkek: Detailed (24 Hours)',
                  font=('Monteserrat SemiBold', 24)).pack(anchor='n', pady=(15, 0))

        hourly_time_frame = CTkFrame(detailed_24h_frame, fg_color="transparent")
        hourly_time_frame.pack(anchor="n", pady=(60, 0), padx=(39, 0))

        weather = ["Second/Weathers/Cloudy.png", "Second/Weathers/Lighting.png", "Second/Weathers/Wind.png", "Second/Weathers/Sun-clouds.png",
                   "Second/Weathers/Rainning.png", "Second/Weathers/Sun-clouds.png", "Second/Weathers/Sun-ontheclouds.png", "Second/Weathers/Cloudy.png"]

        for i in range(6, 14):
            CTkLabel(hourly_time_frame, text=f"{i}:00", font=('Monteserrat SemiBold', 32)).grid(column=i-6, row=0, padx=(0, 84))
        for i in range(8):
            weather_image = CTkImage(Image.open(weather[i]), size=(80, 80))
            CTkLabel(hourly_time_frame, image=weather_image, text="").grid(column=i, row=1, padx=(0, 84), pady=(12, 0))
        for i in range(8):
            CTkLabel(hourly_time_frame, text="15°C", font=('Monteserrat', 24)).grid(column=i, row=2, padx=(0, 84), pady=(12, 0))


class Lower_Frame(CTkFrame):
    def __init__(self, master):
        font = CTkFont('Monteserrat', 24)
        font_semibold = CTkFont('Monteserrat SemiBold', 28)
        pady=(30, 0)
        color = "#B8B7CF"
        ...
        weather = ["Second/Weathers/Cloudy.png", "Second/Weathers/Lighting.png", "Second/Weathers/Wind.png",
                   "Second/Weathers/Rainning.png", "Second/Weathers/Sun-clouds.png",
                   "Second/Weathers/Sun-ontheclouds.png"]
        ...
        for i in range(6):
            ...
            weather_frame = CTkFrame(self, fg_color="transparent")
            weather_frame.grid(column=2, row=i + 1, pady=pady)
            weather_image = CTkImage(Image.open(weather[i]), size=(56, 56))
            ...
            if i == 0:
                ...
                min_max_frame = CTkFrame(self, fg_color="transparent")
